...
- ZIP export/import (points.csv + tags + photos) via the system file picker.
- Dark theme toggle.
...
- Export or back up your data regularly, especially before updating.
- Manual point saves and notification quick add require a fresh accurate fix.
- Location accuracy depends on device sensors, GPS/network availability, Android location services, and surrounding conditions.
- Noise capture requires microphone permission and stores approximate dBFS values, not calibrated sound pressure levels.
...
## 功能
- 一键打点：记录时间、经纬度、标题、可选备注，以及可用时的定位质量元数据。
...
- 位置与数据保存在本地。
- 请定期导出或备份数据，尤其是在更新前。
- 手动打点和通知快捷打点都会要求新鲜且带精度信息的定位。
- 定位精度受设备传感器、GPS/网络条件、Android 定位服务及周围环境影响。
- 噪声采集需要麦克风权限，记录的是近似 dBFS 数值，不是经过校准的声压级。
