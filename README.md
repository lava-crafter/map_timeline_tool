...
- Manual point saves require a fresh accurate fix. Quick add and auto add may use best-effort locations if a precise fix is unavailable.
...
- 位置与数据保存在本地。
- 请定期导出或备份数据，尤其是在更新前。
- 手动打点会要求新鲜且有精度信息的定位；通知快捷打点和自动打点在无法获得精确定位时可能使用尽力而为的位置。
