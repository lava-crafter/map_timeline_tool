...
# Extract all imports
imports = re.findall(r'^import .*$', orig, re.MULTILINE)
orig = re.sub(r'^import .*$\n', '', orig, flags=re.MULTILINE)

new_imports = """import java.io.File
...
all_imports = new_imports + '\n'.join([i for i in imports if 'java.io.File' not in i]) + "\n\n"
...
new_orig = orig.replace('namespace = "com.lavacrafter.maptimelinetool"', 'namespace = "com.lavacrafter.maptimelinetool"\n    sourceSets {\n        getByName("main").res.srcDir(layout.buildDirectory.dir("generated/oss_menu_resources/res").get().asFile)\n    }')

final_file = all_imports + script + new_orig + suffix
